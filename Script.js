...
{id:"sh3", img:"/Dirty55/sh00.webp", name:"نص شواية سادة", basePrice:20, availableIn: ['branch1', 'branch2', 'branch3'], options:[
    {name:"", price:0} 
]},
...
/* ====== متغيرات PWA و SearchBar & Loading ====== */
...
// 🆕 عناصر التحميل وإعادة التحميل
const menuList = document.getElementById('menuList');
const loadingOverlay = document.getElementById('loadingOverlay');
const reloadBtn = document.getElementById('reloadBtn'); 
// 🆕 ربط زر إعادة التحميل بوظيفة تحديث الصفحة
if (reloadBtn) {
    reloadBtn.addEventListener('click', () => {
        // بدلاً من window.location.reload()، سنستخدم دالة التحديث مع مؤشر التحميل
        renderSections();
    });
}
...
function showLoading() {
    menuList.style.opacity = '0.3'; // تعتيم القائمة
    loadingOverlay.style.display = 'flex'; // إظهار المؤشر
}

function hideLoading() {
    menuList.style.opacity = '1'; // إزالة التعتيم
    loadingOverlay.style.display = 'none'; // إخفاء المؤشر
}


/* ====== Render sections ====== */
function renderSections(){
    // 🆕 إظهار مؤشر التحميل قبل بدء العمل
    showLoading(); 
    ...
    // 🆕 إخفاء مؤشر التحميل بعد الانتهاء من عرض القائمة
    setTimeout(() => {
        renderMenu(currentSection);
        hideLoading();
    }, 300); // تأخير بسيط لمحاكاة وقت التحميل
...
function renderMenu(sectionName, searchTerm = ''){
    ...
    // 🆕 يتم إظهار التحميل قبل الدخول إلى هذه الدالة إذا تم استدعاؤها من زر القسم
    menuList.innerHTML='';
    ...
    if(filteredItems.length === 0 && normalizedSearch.length > 0) {
        ...
        // 🆕 إخفاء التحميل إذا لم تكن هناك نتائج
        hideLoading();
        return;
    }
    ...
    if (filteredItems.length === 0 && normalizedSearch.length === 0 && sectionName !== "الكل") {
        ...
        // 🆕 إخفاء التحميل إذا كان القسم فارغاً
        hideLoading();
        return;
    }
    ...
    // 🆕 إخفاء التحميل إذا كانت هذه الدالة قد استُدعيت بشكل مباشر وليس من خلال renderSections
    if (!searchTerm && !document.querySelector('.sec-card.active')) {
        hideLoading(); 
    }
}
...
searchBar.addEventListener('input', (e) => {
    // 🆕 إظهار التحميل عند البحث
    showLoading(); 
    // تأخير بسيط لتحسين تجربة المستخدم أثناء الكتابة
    setTimeout(() => {
