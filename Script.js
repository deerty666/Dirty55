// Script.js (الملف الرئيسي والمعدل)
// ----------------------------------------------------

// 1. استيراد بيانات الفروع والقائمة من الملفات المنفصلة
// تم تعديل المسار إلى '/Dirty55/' لضمان عمل الوحدات (Modules) على الخادم بشكل صحيح
import { BRANCH_CONFIG } from '/Dirty55/branchConfig.js';
import { menuData } from '/Dirty55/menuData.js';


/* ====== متغير لتحديد الفرع الحالي من الرابط (بقي هنا لأنه يعتمد على DOM) ====== */
...
function onSuccess(position) {
    ...
    // إنشاء رابط لموقع جوجل ماب (تم تحديث الرابط ليكون صحيحاً)
...
### 🚨 الخطوة الإلزامية بعد التعديل

بعد رفع هذا الملف (Script.js) إلى الخادم:

1.  **امسح بيانات الموقع** في متصفحك أو جوالك (كما شرحت سابقاً).
2.  **حدث الصفحة مرتين** في متصفحك.

إذا لم يعمل التطبيق بعد هذا التعديل ومسح الكاش، فإن المشكلة تكون في نقطة واحدة فقط: **إعدادات الخادم (MIME Type)** التي ترفض ملفات `js` المستوردة كنظام وحدات (`type="module"`). في هذه الحالة، سنحتاج للبحث عن طريقة لتعديل إعدادات الخادم الخاص بك للسماح بـ `application/javascript`.

هل يمكنك الآن تحميل الملف المعدل واختباره بعد مسح الكاش؟ 🧐
