...
/* ====== متغيرات النافذة المنبثقة للإشعارات (Soft Prompt) ====== */
...
/* ====== منطق الإشعارات Soft Prompt ====== */

function showNotificationPrompt() {
    // نتحقق من دعم المتصفح ومن أن المستخدم لم يرفض الإذن بشكل دائم
    ...
    if (Notification.permission === 'default') {
        // إذا كانت الحالة "default"، لم يسبق للمستخدم أن رفض أو قبل - نعرض النافذة الخاصة بنا
        notificationPrompt.style.display = 'flex';
    }
}

allowNotificationsBtn.onclick = () => {
    // 1. إخفاء النافذة المنبثقة المخصصة
    notificationPrompt.style.display = 'none';

    // 2. طلب الإذن الرسمي من المتصفح/النظام
    Notification.requestPermission().then(permission => {
        if (permission === 'granted') {
            // alert('تم تفعيل الإشعارات بنجاح! يمكنك إزالة هذه الرسالة في النسخة النهائية.');
            // هنا يجب عليك إرسال token المستخدم إلى خادمك لإرسال الإشعارات إليه لاحقاً
        } else {
...
// تشغيل النافذة المنبثقة لطلب الإشعارات بعد 5 ثوانٍ
