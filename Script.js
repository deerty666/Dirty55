...
const menuData = [
    ...
    { 
        ...
        sectionAvailableIn: ['branch1', 'branch2'], // 🏆 تم إضافة توافر القسم
        items:[
            // الوجبة 1
            {id:"eid1", img:"/Dirty55/ed01.webp", name:"ملوخيه", basePrice:10, availableIn: ['branch1', 'branch2'], options:[{name:"صحن", price:0}]}, // 🏆 تم التعديل
            // الوجبة 2
            {id:"eid2", img:"/Dirty55/ed02.webp", name:"باميه", basePrice:10, availableIn: ['branch1', 'branch2'], options:[{name:"صحن", price:0}]}, // 🏆 تم التعديل
            // الوجبة 3
            {id:"eid3", img:"/Dirty55/ed03.webp", name:"مشكل خضار", basePrice:10, availableIn: ['branch1', 'branch2'], options:[{name:"صحن", price:0}]}, // 🏆 تم التعديل
            // الوجبة 4
            {id:"eid4", img:"/Dirty55/ed04.webp", name:"مشكل فران", basePrice:10, availableIn: ['branch1', 'branch2'], options:[{name:"صحن", price:0}]}, // 🏆 تم التعديل
            // الوجبة 5
            {id:"eid5", img:"/Dirty55/ed05.webp", name:"مسقع", basePrice:12, availableIn: ['branch1', 'branch2'], options:[{name:"صحن", price:0}]} // 🏆 تم التعديل
        ]
    },
    { 
        section:"المقبلات", 
...
// 🚀 NEW: متغيرات الموقع الجديد
const manualLocationInput = document.getElementById('manualLocation'); 
const sendLocationBtn = document.getElementById('sendLocationBtn');
let locationLink = null; // لتخزين رابط GPS بعد الحصول عليه
...
// 🚀 NEW: ربط زر إرسال الموقع بدالة Geolocation
...
sendWhatsapp.addEventListener('click', () => {
    ...
    const deliveryType = document.querySelector('input[name="deliveryType"]:checked')?.value;
    const locationNote = manualLocationInput.value.trim(); // 🚀 NEW: جلب قيمة العنوان اليدوي
    ...
    // 🚀 NEW: إضافة الموقع (هنا يتم استخدام رابط GPS أو النص اليدوي)
    if (deliveryType === 'delivery' && (locationLink || locationNote)) {
        if (locationLink) {
            lines.push(`4. موقع التوصيل (GPS): ${locationLink}`);
        } else if (locationNote) {
            lines.push(`4. ملاحظات الموقع/العنوان: ${locationNote}`);
        }
    }
    ...
    // إعادة تعيين حالة الموقع بعد الإرسال
    locationLink = null;
...
// 🚀 NEW: دالة إرسال الموقع عبر GPS
// ------------------------------------------
function sendLocation() {
    // 1. تغيير حالة الزر للإشارة إلى الانتظار
    sendLocationBtn.disabled = true;
    sendLocationBtn.innerText = 'جاري تحديد الموقع... 🛰️';
    sendLocationBtn.style.backgroundColor = 'var(--gold)'; 

    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            // 2. دالة النجاح (Success)
            (position) => {
                const lat = position.coords.latitude;
                const lon = position.coords.longitude;
                // إنشاء رابط خرائط جوجل (تم تصحيح الرابط)
                const link = `https://maps.google.com/maps?q=$${lat},${lon}`; // 🏆 تم التصحيح هنا
                
                locationLink = link;
                
                // تحديث واجهة المستخدم
                sendLocationBtn.innerText = 'تم تحديد الموقع بنجاح ✅';
                sendLocationBtn.style.backgroundColor = '#4CAF50'; // أخضر
                manualLocationInput.value = `📍 الموقع التلقائي (GPS):\n${locationLink}`; // 🏆 تم التصحيح هنا
                sendLocationBtn.disabled = false;
                
                setTimeout(() => {
                    sendLocationBtn.innerText = '📍 إرسال موقعي الحالي (GPS)';
                    sendLocationBtn.style.backgroundColor = 'var(--gold)';
                }, 4000);
            },
            // 3. دالة الفشل (Error)
            (error) => {
                let errorMessage = 'تعذر تحديد الموقع. يرجى تفعيل GPS أو إدخال العنوان يدوياً.';
                if (error.code === error.PERMISSION_DENIED) {
                    errorMessage = 'تم رفض إذن تحديد الموقع. يرجى السماح له من إعدادات المتصفح.';
                }
                
                manualLocationInput.value = errorMessage;
                sendLocationBtn.innerText = 'حدث خطأ ⛔';
                sendLocationBtn.style.backgroundColor = 'var(--red)';
                sendLocationBtn.disabled = false;
                locationLink = null; // مسح أي رابط سابق
        ...
        manualLocationInput.value = 'المتصفح لا يدعم تحديد الموقع (Geolocation غير متوفر).';
        sendLocationBtn.innerText = 'خطأ المتصفح ❌';
        sendLocationBtn.disabled = false;
    }
}
// ------------------------------------------
