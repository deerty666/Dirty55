// ====================================================================
// 🔧 التعديل الجديد: تحديد المسار الأساسي للمستودع (Dirty55)
// بناءً على مسارات الصور لديك (مثل /Dirty55/sh00.webp)، هذا هو جذر المستودع.
const REPO_BASE_PATH = '/Dirty55';

// دالة لمساعدتنا في بناء رابط مطلق صحيح على GitHub Pages
function getRedirectPath(filename, params) {
    // نستخدم window.location.origin للحصول على https://[user].github.io
    // ثم نضيف المسار الأساسي للمستودع واسم الملف والمتغيرات
    return `${window.location.origin}${REPO_BASE_PATH}/${filename}?${params}`;
}
// ====================================================================

// 🔧 التعديل 3 المُصحَّح (باستخدام الدالة الجديدة)
// إعادة توجيه تلقائي لو تم فتح الصفحة بدون فرع
if (!location.search.includes("branch=")) {
    // 🛑 نستخدم المسار المطلق المُنشأ حديثاً
    window.location.href = getRedirectPath('branch_selector.html', 'direct=1');
    // نوقف التنفيذ مباشرة
    throw new Error('Redirecting due to missing branch parameter.'); 
}
...
const BRANCH_CONFIG = {
    'branch1': { 
        whatsapp: '966536803598', // ⭐️ رقم واتساب فرع الرياض (لبن الاحمدية)
...
/* ====== متغير لتحديد الفرع الحالي من الرابط والتحقق من التوجيه ====== */
...
// ====================================================================
// 🔧 التعديل 2 المُصحَّح (باستخدام الدالة الجديدة)
// إذا لم يتم تحديد فرع أو كان غير صحيح، يتم التوجيه مباشرة لصفحة الاختيار
if (!currentBranchId || !BRANCH_CONFIG[currentBranchId]) {
    // 🛑 نستخدم المسار المطلق المُنشأ حديثاً
    window.location.href = getRedirectPath('branch_selector.html', 'from=index');
    // إيقاف تنفيذ الكود
    return; 
}
// ====================================================================
...
// استكمال الإعدادات بعد التأكد من وجود الفرع
const currentBranch = BRANCH_CONFIG[currentBranchId];
document.title = `قائمة سحايب ديرتي - فرع ${currentBranch.name}`; // تحديث عنوان الصفحة باسم الفرع

/* ====== بيانات المنيو - تم تحديث جميع مسارات الصور ومنطق الإتاحة ====== */
const menuData = [
  // 1. القسم الجديد: الكل
  { 
    section:"الكل", 
  ...
  { 
    ...
    items:[
      // الوجبة 1: تم تعديل مسار الصورة ليتطابق مع صورة القسم الرئيسية (sh00.webp)
    ...
    // الوجبة 2: تم تعديل مسار الصورة ليتطابق مع صورة القسم الرئيسية (sh00.webp)
  ...
  { 
    ...
    items:[
      // الوجبة 1
      {id:"md1", img:"/Dirty55/md01.webp", name:"مظبي", basePrice:46, availableIn: ['branch1', 'branch2', 'branch3'], options:[
      {name:"رز شعبي", price:0},
      {name:"رز بشاور", price:4},
      {name:"رز مندي", price:4},
      {name:"رز مثلوثه", price:4}
    ...
    // الوجبة 2
    {id:"md2", img:"/Dirty55/md02.webp", name:"نص مظبي", basePrice:24, availableIn: ['branch1', 'branch2', 'branch3'], options:[
      {name:"رز شعبي", price:0},
      {name:"رز بشاور", price:1},
      {name:"رز مندي", price:1},
      {name:"رز مثلوثه", price:1}
  ...
  { 
    ...
    items:[
      // الوجبة 1: مندي حبة كامل (تم إخفاؤها من فرع 'branch1' بناءً على طلبك)
      {id:"mn1", img:"/Dirty55/mn01.webp", name:"مندي", basePrice:46, 
        availableIn: ['branch2', 'branch3'], // 🛑 تم إزالة 'branch1' لإخفائها تماماً عن فرع لبن الاحمدية
        options:[ 
        {name:"رز شعبي", price:0},
        {name:"رز بشاور", price:4},
        {name:"رز مندي", price:4},
    ...
    // الوجبة 2
    {id:"mn2", img:"/Dirty55/mn02.webp", name:"نص مندي", basePrice:24, availableIn: ['branch1', 'branch2', 'branch3'], options:[
      {name:"رز شعبي", price:0},
      {name:"رز بشاور", price:1},
      {name:"رز مندي", price:1},
      {name:"رز مثلوثه", price:1}
  ...
  { 
    ...
    // الوجبة 2
    {id:"mdf2", img:"/Dirty55/mf02.webp", name:"نص مدفون", basePrice:24, availableIn: ['branch1', 'branch2', 'branch3'], options:[
      {name:"رز شعبي", price:0},
      {name:"رز بشاور", price:1},
      {name:"رز مندي", price:1},
      {name:"رز مثلوثه", price:1}
  ...
  { 
    ...
    items:[
      // الوجبة 1
      {id:"mq1", img:"/Dirty55/mq01.webp", name:"دجاج مقلوبه حبه", basePrice:50, availableIn: ['branch1', 'branch2', 'branch3'], options:[{name:"رز شعبي", price:0}]},
      // الوجبة 2
      {id:"mq2", img:"/Dirty55/mq02.webp", name:"نص دجاج مقلوبه", basePrice:25, availableIn: ['branch1', 'branch2', 'branch3'], options:[{name:"رز شعبي", price:0}]}
  ]},
  { 
    section:"مضغوط", 
    ...
    items:[
      ...
      // الوجبة 2
      {id:"mg2", img:"/Dirty55/mg02.webp", name:"نص حبه مضغوط", basePrice:25, availableIn: ['branch1', 'branch2', 'branch3'], options:[{name:"رز مضغوط", price:0}]}
  ]},
  { 
    section:"زربيان", 
    ...
    items:[
      // الوجبة 1
      {id:"zb1", img:"/Dirty55/zb01.webp", name:"دجاج زربيان حبه", basePrice:50, availableIn: ['branch1', 'branch2', 'branch3'], options:[{name:"رز زربيان", price:0}]},
      // الوجبة 2
      {id:"zb2", img:"/Dirty55/zb02.webp", name:"نص حبه زربيان", basePrice:25, availableIn: ['branch1', 'branch2', 'branch3'], options:[{name:"رز زربيان", price:0}]}
  ]},
  { 
    section:"قسم اللحوم", 
    sectionImg: "/Dirty55/me00.webp",
    items:[
    // الوجبة 1: تيس مندي كامل
...
/* ====== Render menu - عرض القائمة وتطبيق الخصم الخاص بالفرع ====== */
function renderMenu(sectionName, searchTerm = ''){
  ...
  if(filteredItems.length === 0 && (normalizedSearch.length > 0 || sectionName !== "الكل")) {
      menuList.innerHTML = `<p style="text-align:center; padding: 20px; color: var(--light-text);">لا توجد وجبات متوفرة حالياً تطابق بحثك في فرع ${currentBranch.name}.</p>`;
      return;
  }
...
function addToCart(item){
  ...
  // إنشاء مفتاح فريد لتمييز الوجبات حتى لو كانت بنفس الاسم (بناءً على الملاحظات والخيارات)
...
sendWhatsapp.addEventListener('click', () => {
  ...
  lines.push(`**فرع الطلب:** ${currentBranch.name}`);
  lines.push('---');
  lines.push('**تفاصيل الطلبات:**');
  
  cart.forEach(it=>{
    const price=(it.basePrice || 0)+(it.selectedOption?it.selectedOption.price:0);
    const optionText = it.selectedOption && it.selectedOption.name !== 'نفر' && it.selectedOption.name !== 'طبق' && it.selectedOption.name !== 'عبوة' && it.selectedOption.name !== '' ? ` (${it.selectedOption.name})` : '';
    
    const noteText = it.note ? `\n   - 📝 ملاحظة: ${it.note}` : '';

    lines.push(`${it.qty} × ${it.name}${optionText} - ${price*it.qty} ريال${noteText}`);
    subtotal+=price*it.qty;
  });
  ...
  if(deliveryType==='delivery'){ 
    ...
    lines.push('\n**يرجى إرسال الموقع الحالي للتوصيل**');
  } else {
...
window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault();
  deferredPrompt = e;
  // تأكد أن زر التثبيت موجود في ملف HTML
...
// البدء في عرض القائمة والبيانات فقط إذا كان currentBranch موجوداً (وإلا كان قد تم التوجيه بالفعل)
if (currentBranch) {
    renderSections(); 
    renderCart();
}
