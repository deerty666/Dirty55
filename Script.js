...
/* ====== منطق الإشعارات المنبثقة (Soft Prompt Notification) - جديد ====== */
...
function requestNotificationPermission() {
    Notification.requestPermission().then(permission => {
        ...
        // تعيين علامة في التخزين المحلي لتجنب الإزعاج مرة أخرى
...
allowNotificationsBtn.addEventListener('click', () => {
    // عند النقر على "نعم، أريد ذلك"، نطلب الإذن الفعلي
    requestNotificationPermission();
});

denyNotificationsBtn.addEventListener('click', () => {
    // عند النقر على "لا شكراً"، نخفي النافذة ونضع علامة في التخزين المحلي
    notificationPrompt.style.display = 'none';
    localStorage.setItem('notifications_asked', 'true');
});

// تشغيل التحقق من الإشعارات بعد فترة زمنية قصيرة (مثلاً 5 ثواني)
// هذا يضمن تحميل القائمة أولاً وعدم إزعاج المستخدم فوراً
