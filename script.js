...
// عناصر النافذة المنبثقة للتطبيق الجديدة
...
// إغلاق النافذة عند الضغط خارجها (لأي modal مفتوحة)
window.onclick = function(event) {
    if (event.target == checkoutModal) {
        closeModal();
    }
    if (event.target == installPromptModal) {
        installPromptModal.style.display = 'none';
    }
}

// === منطق إظهار رسالة تثبيت التطبيق مرة واحدة ===

function checkAndShowInstallPrompt() {
    // التحقق مما إذا كان المستخدم قد رأى الرسالة من قبل
    const hasSeenPrompt = localStorage.getItem('appPromptShown');
    
    // إذا لم يرها من قبل، أظهرها
    if (!hasSeenPrompt) {
        // تأخير بسيط لإعطاء العميل فرصة لرؤية القائمة
        setTimeout(() => {
            installPromptModal.style.display = 'block';
            // بمجرد عرضها، قم بتعيين العلامة في التخزين المحلي
            localStorage.setItem('appPromptShown', 'true');
        }, 3000); // تظهر بعد 3 ثوانٍ
    }
}

// إغلاق رسالة تثبيت التطبيق
...
function calculateFinalTotal() {
    ...
    if (selectedShipping === 'delivery') {
        ...
        shippingFee = 0; // لضمان أن الرسوم صفر عند الاستلام
...
function addToCart(event) {
    ...
    const price = parseFloat(menuItem.querySelector('.price').dataset.price); // تأكدنا من استخدام dataset.price
...
// === تسجيل عامل الخدمة (Service Worker) لتمكين ميزة PWA ===
// التعديل الأخير والنهائي لضمان نطاق التحكم الصحيح على GitHub Pages
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        // *** هذا هو السطر الذي تم تعديله ليناسب مسار GitHub Pages ونطاق التحكم: ***
        navigator.serviceWorker.register('/Dirty55/service-worker.js', { scope: '/Dirty55/' })
            .then(registration => {
